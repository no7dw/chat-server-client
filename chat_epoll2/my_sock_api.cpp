...
int EventSet(myevent_s *ev, int fd, int(*call_back)(int, int, void*, int, myevent_s*), void *arg)
{
	ev->fd = fd;
...
int RecvData(int fd, int events, void *arg, int gpollfd, myevent_s *gevents);
int SendData(int fd, int events, void *arg, int gpollfd, myevent_s *gevents);
...
int RecvData(int fd, int events, void *arg, int gpollfd, myevent_s *gevents)
{
	...
	if(readback > 0 )
	{
		...
		EventAdd(gpollfd, EPOLLOUT|EPOLLET, ev);
	}
	else if(readback == 0)
	{
		close(ev->fd);
...
int SendData(int fd, int events, void *arg,  int gpollfd, myevent_s *gevents)
{
	...
	if(writeback > 0)
	{
		...
		EventAdd(gpollfd, EPOLLIN|EPOLLET, ev);
	}
	else
	{
		close(ev->fd);
...
int AcceptConn(int fd, int evetns, void *arg, int gpollfd, myevent_s *gevents)
{
	...
	do{
		...
		{
			if(gevents[index].status == 0)//not in list
			{
				break;
			}
			if(index == MAX_EVENTS)
			...
			EventSet(&gevents[index] , clientfd, RecvData, &gevents[index]);
			EventAdd(gpollfd, EPOLLIN|EPOLLET, &gevents[index]);
			printf("new connection[%s:%d] at [time:%d]\n", inet_ntoa(sin.sin_addr),
				ntohs(sin.sin_port),(int) gevents[index].last_active);

		}//for
	}while(0);
}

int sock_server_init_epoll(int &listenfd , uint16_t server_port, myevent_s *gevents)
{
	listenfd = sock_server_init(listenfd, server_port);
	EventSet(&gevents[MAX_EVENTS], listenfd, AcceptConn, &gevents[MAX_EVENTS]);
	EventAdd(listenfd, EPOLLIN|EPOLLET, &gevents[MAX_EVENTS]);
	return listenfd;
}
*/
